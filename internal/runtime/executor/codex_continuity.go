...
func stableCodexContinuityKey(seed string) string {
	hash := sha1.Sum([]byte(seed))
	return "codex-cache-" + hex.EncodeToString(hash[:])
}
	...
	if ginCtx := ginContextFrom(ctx); ginCtx != nil {
		...
		if v, exists := ginCtx.Get("apiKey"); exists && v != nil {
			if trimmed := principalString(v); trimmed != "" {
				return codexContinuity{Key: stableCodexContinuityKey("cli-proxy-api:codex:prompt-cache:" + trimmed), Source: "client_principal"}
			}
		}
	}
	if auth != nil {
		if authID := strings.TrimSpace(auth.ID); authID != "" {
			return codexContinuity{Key: stableCodexContinuityKey("cli-proxy-api:codex:prompt-cache:auth:" + authID), Source: "auth_id"}
		}
	}
